    ...
    private static LoginInfo queryLoginInfo(LoginInfo info) {
        try {
            ...
            if (loginCallback.contains("loginCallback")) {
                ...
                info.setUsername(obj.toJSONString("username"));
                info.setPuid(obj.toJSONString("puid"));
                info.setHeadurl(obj.toJSONString("headurl"));
                info.setStatus(obj.getIntValue("status"));
            }
            return info;
        } catch (IOException e) {
            e.printStackTrace();
