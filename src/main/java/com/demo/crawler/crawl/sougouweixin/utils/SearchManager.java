package com.demo.crawler.crawl.sougouweixin.utils;
...
public class SearchManager {
    ...
    public static PageData<OfficialAccount> searchOA(PageData<OfficialAccount> req) throws IOException {
        ...
        for (Element e : elements) {
            ...
            String avatarImgsrc = e.selectFirst("div[class=img-box]").selectFirst("img").attr("src");
            oa.setAvatar(avatarImgsrc);
    ...
    public static PageData<Article> searchArticle(PageData<Article> req) throws IOException, InterruptedException {
        ...
        log.info(String.valueOf(req.getPageNum()));
        log.info(String.valueOf(req.getPageSize()));
        log.info("************ doc end ******************* ");
        ...
        log.info("延时3000ms +");
        //Thread.sleep(3000);
        HttpCrawlerUtils.sleepRandom(3, 10);

        log.info("模拟点开文章标题，提取真实URL");
        getRealUrl(res, req);
    ...
    private static List<Article> getArticles(Document doc) {
        ...
        Elements elements = doc.selectFirst("ul[class=news-list]").select("li");
        for (Element e : elements) {
            Article article = new Article();
            SendSpriderDto sendSpriderDto = new SendSpriderDto();

            Element elementTxtBox = e.selectFirst("div[class=txt-box]");

            Element a = elementTxtBox.selectFirst("h3").selectFirst("a");
            article.setOriginalUrl("https://weixin.sogou.com" + a.attr("href"));
            article.setTitle(a.text());

            a = elementTxtBox.selectFirst("div[class=s-p]").selectFirst("a[class=account]");
            article.setOaName(a.text());
            article.setTextBox(e.outerHtml());

            String createDateStr = elementTxtBox.selectFirst("div[class=s-p]").attr("t");
            Timestamp createDate = new Timestamp(Long.valueOf(createDateStr + "000"));

            String imgMainStr = e.selectFirst("div[class=img-box]").selectFirst("img").attr("src");

            sendSpriderDto.setTITLE(article.getTitle());
            sendSpriderDto.setAUTHOR(article.getOaName());
            sendSpriderDto.setMAIN_TEXT(article.getTextBox());
            sendSpriderDto.setMAIN_DATE(createDate);
            sendSpriderDto.setMAIN_PIC_URL(imgMainStr);
            sendSpriderDto.setFROMSITE("微信公众号");
            sendSpriderDto.setPARENT_URL(doc.location());

            article.setSendSprider(sendSpriderDto);
...
//        }
    ...
    private static void approveSearch(Connection.Response res, PageData<Article> req) throws IOException {
        ...
        Connection.Response resp = connection.execute();
        log.info("search = " + resp.statusCode());
    }
...
//            System.out.println(doc);
            ...
            for (Element e : elements) {
                if (e.html().contains("var url =")) {
                    ...
                    SendSpriderDto sendSpriderDto =  article.getSendSprider();
                    openPage(urlWithSuffix, realUrl, req, sendSpriderDto);

                    //TODO 设置微信的其他信息
                    sendSpriderDto.setURL(realUrl);

                    //req.addSendList(sendSpriderDto);
            ...
            if (hasResult) {
                ...
                if (currentPos == req.getRecords().size() - 1) {
                    log.info("已转换当前页面： - " + req.getPageNum() + " - 最后一条URL");
...
//                    System.out.println("拿到" + page + "页数据");
                    ...
                    log.info("休眠" + TIME_DELAY + "ms");
                    //Thread.sleep(TIME_DELAY);
                    HttpCrawlerUtils.sleepRandom(5, 15);
                    log.info("转换下一条URL");
                }
            } else {
                log.warn("出现异常");
    ...
    private static void approveOuter(Connection.Response res, PageData<Article> req) throws IOException {
        ...
        Connection connection = Jsoup.connect(approve_url)
                ...
                .method(Connection.Method.GET);
        Connection.Response resp = connection.execute();
        log.info("outer = " + resp.statusCode());
    }
...
//        System.out.println(resp.parse().toString());
    ...
    private static void approveInner(String approveUuid, String approveToken, String urlWithSuffix, PageData<Article> req) throws IOException {
        ...
        Connection connection = Jsoup.connect(approve_url)
                ...
                .method(Connection.Method.GET);
        Connection.Response resp = connection.execute();
        log.info("inner = " + resp.statusCode());
    }
     ...
     * @throws IOException
     */
    private static void openPage(String urlWithSuffix, String realUrl, PageData<Article> req, SendSpriderDto sendSpriderDto) throws IOException {
        Map<String, String> headers = new HashMap<String, String>();
        headers.put("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8");
        headers.put("Accept-Encoding", "gzip, deflate, br");
        headers.put("Accept-Language", "zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2");
       ...
       //getWxArticlesSendSprider(doc, sendSpriderDto);
        sendSpriderDto.setALL_TEXT(doc.html());

        String jsArticle = doc.select("#js_article").outerHtml();
        //System.out.println(jsArticle);

        log.info("openPage：" + res.statusCode());
    }
     ...
     * @return
     */
    private static SendSpriderDto getWxArticlesSendSprider(Document doc, SendSpriderDto sendSpriderDto) {

        sendSpriderDto.setALL_TEXT(doc.html());

        log.error(" \n\n获取的文章 ：  ");
        //log.info(JSON.toJSONString(sendSpriderDto));
